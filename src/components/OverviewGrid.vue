<template>
  <div class="overview-container">
    ...
    <div v-else class="grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-4">
      <SiteCard v-for="site in sortedSites" :key="site.id" :siteId="site.id" :host="site.host" :visitData="site.visit" :echartsData="site.echarts" :loading="false" :pinned="pinnedStore.isPinned(site.id)" @select="(id: string) => $emit('select-site', id)" @pin="pinnedStore.pin" @unpin="pinnedStore.unpin" />
    </div>
  </div>
</template>
...
const sortedSites = computed(() => {
  ...
  if (q) {
    list = props.sites.filter((s) => s.id.toLowerCase().includes(q) || (s.host && s.host.toLowerCase().includes(q)));
  }
