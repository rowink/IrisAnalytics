<template>
  <Card class="site-card group relative cursor-pointer transition-all duration-150 hover:shadow-md hover:border-[#4f6ef7]/30" @click="$emit('select', siteId)">
    <CardHeader class="pb-3">
      <div class="flex items-center gap-2">
        <img v-if="host && !faviconFailed" class="w-4 h-4 rounded" :src="`https://icons.duckduckgo.com/ip3/${host}.ico`" referrerpolicy="no-referrer" @error="faviconFailed = true" />
      ...
      <div v-if="!loading" class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
        <button v-if="pinned" class="p-1 rounded-md hover:bg-[#f4f4f5] dark:hover:bg-[#27272a] text-[#4f6ef7] transition-colors" :title="'取消置顶'" @click.stop="$emit('unpin', siteId)">
          <PinOff class="w-4 h-4" />
        </button>
        <button v-else class="p-1 rounded-md hover:bg-[#f4f4f5] dark:hover:bg-[#27272a] text-[#a1a1aa] hover:text-[#4f6ef7] transition-colors" :title="'置顶'" @click.stop="$emit('pin', siteId)">
          <Pin class="w-4 h-4" />
        </button>
      </div>
    </CardHeader>
