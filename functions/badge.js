 ...
 * 动态 SVG 徽标
 *
 * GET /badge?siteID=<id>&time=<period>&style=<uv|pv>&label=<title>
 *   - siteID: 网站标识（必填，仅允许字母/数字/-/_）
...
const SITEID_PATTERN = /^[a-zA-Z0-9_-]{1,64}$/;
...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    // 参数校验
    if (!siteID || !SITEID_PATTERN.test(siteID)) {
    ...
    // 查询统计数据（uv=访客数，pv=浏览量）
    const tz = (request.cf && request.cf.timezone) || "Asia/Shanghai";
    const query = `SELECT SUM(_sample_interval) AS views, SUM(IF(double1 = '1', double1, 0.0)) AS visitor, ` + `SUM(IF(double2 = '1', double2, 0.0)) AS visit ` + `FROM AnalyticsDataset WHERE timestamp >= ${formatTime(time, tz)} AND blob1 = '${siteID}'`;
    ...
    const value = style === "pv" ? row.views : row.visitor;
